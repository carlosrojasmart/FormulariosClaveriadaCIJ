...
with tab1:
    ...
    if stage == 1:
        with st.form("form_participante_stage1", clear_on_submit=False):
            ...
            doc_contacto = st.text_input(
                "Documento del contacto (solo dígitos)",
                max_chars=20,
                placeholder="Ej: 1012345678",
                key="part_doc_a"
            )
            nom_contacto = st.text_input("Nombres del contacto", key="part_nom_a")
            ape_contacto = st.text_input("Apellidos del contacto", key="part_ape_a")
            tel_contacto = st.text_input("Teléfono del contacto", key="part_tel_a")
            correo_contacto = st.text_input("Correo del contacto (opcional)", key="part_correo_a")
